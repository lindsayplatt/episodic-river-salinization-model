...
p6_targets <- list(
  ...
  # Simplify polygons to 1 km (speeds everything up)
  tar_target(p6_state_huc_sf_simple, st_simplify(p6_state_huc_sf, dTolerance = 1000) %>% 
               st_union()), #HD
  
  # Now for each HUC-02 that intersected with our states, 
  # Avoids using identify_comids_aoi to because we have all the data and don't need to use nhdtools
  tar_target(huc_intersect, st_intersects(p1_nhdplus_flowlines_sf %>% 
                                  dplyr::select(COMID, FromNode, ToNode, Divergence, FTYPE,
                                                AreaSqKM, LENGTHKM, GNIS_ID), 
                                p6_state_huc_sf_simple %>% st_transform(crs(p1_nhdplus_flowlines_sf)))),
  
  tar_target(huc_intersect.indx, which(as.numeric(huc_intersect) == 1)),
  
  tar_target(huc_flowlines, p1_nhdplus_flowlines_sf[huc_intersect.indx,] %>% 
    dplyr::select(COMID, FromNode, ToNode, Divergence, FTYPE, AreaSqKM, 
                  LENGTHKM, GNIS_ID,StreamOrde) %>% 
    hydroloom::add_toids() %>% 
      rename(nhd_comid = COMID)),
  
  tar_target(huc_flowlines_comid, huc_flowlines %>% st_drop_geometry() %>% 
              select(nhd_comid)),
  
  tar_target(p6_state_comids, huc_flowlines_comid %>% 
               mutate(region = lonlat_to_state(st_endpoint(huc_flowlines))) %>%
  ...
  #            identify_comids_aoi(p6_state_huc_sf_simple) %>% 
  #              mutate(region = p6_state_huc_sf_simple$name,
  #                     region_fname = p6_state_huc_sf_simple$huc2) %>% 
  #              st_drop_geometry() %>% 
  #              select(nhd_comid = comid, region, region_fname),
  #            pattern = map(p6_state_huc_sf_simple)),
  # 
  ...
  tar_target(p6_huc_flowlines_sf, p1_nhdplus_flowlines_sf %>%
               hydroloom::add_toids() %>%
               dplyr::select(COMID, FromNode, ToNode, Divergence, FTYPE,
                             AreaSqKM, LENGTHKM, GNIS_ID, StreamOrde) %>%
               filter(COMID %in% huc_flowlines_comid$nhd_comid)),
  ...
  tar_target(p6_index_ids, make_index_ids(p6_huc_flowlines_network_tbl)),
  tar_target(p6_froms, make_fromids(p6_index_ids)),
  ...
  # Group comids because it exhausts system memory if you don't 
  tar_target(p6_nhdplus_attr_vals_tbl_list, p6_nhdplus_attr_vals_tbl %>% 
               mutate(nhd2 = round(nhd_comid,-4)) %>% 
               group_by(nhd2) %>% 
               mutate(tar_group = cur_group_id()) %>% 
               ungroup() %>% 
               dplyr::select(-nhd2) %>% group_by(tar_group) %>% group_split()),
  
  # Map over groups, otherwise exhausts memory limits 
  tar_target(p6_attr_nhd, {
          prepare_nhd_attributes(p6_nhdplus_attr_vals_tbl_list, NULL)
          }, pattern = map(p6_nhdplus_attr_vals_tbl_list)),
  ...
  # Group comids because it exhausts system memory if you don't 
  tar_target(p6_upstream_comids_grp, p6_upstream_comids %>% 
               mutate(nhd2 = round(nhd_comid,-4)) %>% 
               group_by(nhd2) %>% 
               mutate(tar_group = cur_group_id()) %>% 
               ungroup() %>% 
  ...
  # Should be about 2000 groups for entire region
  tar_target(p6_upstream_comids_grp_list, 
             p6_upstream_comids_grp %>% group_by(tar_group) %>% group_split()),
  
  # Map over groups, otherwise exhausts memory limits 
  tar_target(p6_attr_basinArea, {
                            calculate_catchment_areas(polys_sf = p6_nhdplus_catchment_sf,
                                                      comid_upstream_tbl = p6_upstream_comids_grp_list,
                                                      comid_site_xwalk = NULL)
    
  }, pattern = map(p6_upstream_comids_grp_list)),
  ...
  #                                                         comid_upstream_tbl = p6_upstream_comids_grp,
  #                                                         comid_site_xwalk = NULL),
